...
const I18N = {
  en: {
    today: "Today", all: "All", no_media: "No media found.",
    no_media_day: "No media for this day.", event: "event", events: "events",
    selected: "selected", delete: "Delete", cancel: "Cancel",
    select: "Select", stop_select: "Stop selecting", download: "Download",
    prev_day: "Previous day", next_day: "Next day", selected_day: "Selected day",
    confirm_delete: (n) => `Are you sure you want to delete ${n} file(s)?`,
  },
  nl: {
    today: "Vandaag", all: "Alles", no_media: "Geen beelden gevonden.",
    no_media_day: "Geen beelden voor deze dag.", event: "event", events: "events",
    selected: "geselecteerd", delete: "Verwijderen", cancel: "Annuleren",
    select: "Selecteren", stop_select: "Selectie stoppen", download: "Download",
    prev_day: "Vorige dag", next_day: "Volgende dag", selected_day: "Geselecteerde dag",
    confirm_delete: (n) => `Weet je zeker dat je ${n} bestand(en) wilt verwijderen?`,
  },
  de: {
    today: "Heute", all: "Alle", no_media: "Keine Medien gefunden.",
    no_media_day: "Keine Medien für diesen Tag.", event: "Ereignis", events: "Ereignisse",
    selected: "ausgewählt", delete: "Löschen", cancel: "Abbrechen",
    select: "Auswählen", stop_select: "Auswahl beenden", download: "Herunterladen",
    prev_day: "Vorheriger Tag", next_day: "Nächster Tag", selected_day: "Ausgewählter Tag",
    confirm_delete: (n) => `Möchten Sie ${n} Datei(en) wirklich löschen?`,
  },
  fr: {
    today: "Aujourd'hui", all: "Tout", no_media: "Aucun média trouvé.",
    no_media_day: "Aucun média pour ce jour.", event: "événement", events: "événements",
    selected: "sélectionné(s)", delete: "Supprimer", cancel: "Annuler",
    select: "Sélectionner", stop_select: "Arrêter la sélection", download: "Télécharger",
    prev_day: "Jour précédent", next_day: "Jour suivant", selected_day: "Jour sélectionné",
...
(() => {
  const candidates = [
    "hui-masonry-view", "hui-view", "ha-panel-lovelace",
    "hc-lovelace", "hui-entities-card", "ha-card",
  ];
...
class CameraGalleryCard extends LitElement {
  ...
  _normPrefix(p) {
    ...
    if (!s) return DEFAULT_DELETE_PREFIX;
  ...
  setConfig(config) {
    ...
    const delete_service =
      (config.shell_command && config.shell_command.trim()) ||
      (config.delete_service && config.delete_service.trim()) ||
      DEFAULT_DELETE_SERVICE;

    // ✅ media_path alias removed — sensor bepaalt media, prefix is alleen veiligheidscheck
    const delete_prefix = this._normPrefix(
      str(config.delete_prefix, DEFAULT_DELETE_PREFIX)
    );
    ...
    this.config = {
      ...
      delete_service,   // ← moet hier staan
      delete_confirm,
    };
  ...
  async _downloadSrc(src) {
    ...
    const name = (() => {
      try { return decodeURIComponent(base.split("/").pop() || "download"); }
      catch (_) { return base.split("/").pop() || "download"; }
    })();
  ...
  async _bulkDelete(selectedSrcList) {
    ...
    const prefix = this._normPrefix(this.config?.delete_prefix || DEFAULT_DELETE_PREFIX);
  ...
  _captureFirstFrame(src) {
    return new Promise((resolve, reject) => {
      ...
      v.muted = true; v.playsInline = true; v.preload = "metadata"; v.controls = false;
      const cleanup = () => { try { v.pause(); } catch (_) {} v.removeAttribute("src"); try { v.load(); } catch (_) {} };
      ...
      const draw = () => {
        try {
          ...
          cleanup(); resolve(c.toDataURL("image/jpeg", 0.72));
        } catch (e) { fail(e); }
      };
      v.addEventListener("error", () => fail(new Error("video load error")), { once: true });
      v.addEventListener("loadedmetadata", () => { try { v.currentTime = 0.01; } catch (_) { draw(); } }, { once: true });
  ...
  _formatDateTime(dtKey) {
    if (!dtKey) return "";
    try {
      const loc = this._hass?.language || "en";
      const dt = new Date(dtKey);
      const date = new Intl.DateTimeFormat(loc, { day: "2-digit", month: "short" }).format(dt).replace(".", "");
      const time = new Intl.DateTimeFormat(loc, { hour: "2-digit", minute: "2-digit" }).format(dt);
      return `${date} • ${time}`;
    } catch (_) { return ""; }
  }
  ...
  _formatDay(dayKey) {
    ...
    try {
      const loc = this._hass?.language || "en";
      return new Intl.DateTimeFormat(loc, { day: "2-digit", month: "long" })
        .format(new Date(`${dayKey}T00:00:00`)).replace(".", "");
  ...
  _stepDay(delta, days, activeDay) {
    ...
    this._selectedDay = next; this._filterAll = false; this._selectedIndex = 0;
    this._scrubMinute = NaN; this._pendingScrollToI = 0; this._exitSelectMode(); this.requestUpdate();
  }
  ...
  render() {
    ...
    if (!rawItems.length) return html`<div class="empty">${this._t("no_media")}</div>`;

    const withDt = rawItems.map((src, idx) => ({
      src, idx, dtMs: this._dtMsFromSrc(src), dayKey: this._extractDayKey(src),
    }));

    withDt.sort((a, b) => {
      const aOk = Number.isFinite(a.dtMs), bOk = Number.isFinite(b.dtMs);
    ...
    const filteredAll = !activeDay || this._filterAll
      ? allWithDay : allWithDay.filter((x) => x.dayKey === activeDay);

    if (!filteredAll.length) return html`<div class="empty">${this._t("no_media_day")}</div>`;
    ...
    const tsPosClass =
      this.config.bar_position === "bottom" ? "bottom" :
      this.config.bar_position === "hidden" ? "hidden" : "top";
        ...
        <div class="panel" style="width:${PREVIEW_WIDTH}; margin:0 auto;">
          ...
          >
            ${this._isVideo(selected)
              ? html`<video class="pimg" src=${selected} controls playsinline preload="auto"
                  poster=${this._posterCache.get(selected) || ""}></video>`
              : html`<img class="pimg" src=${selected} alt="" />`}

            ${tsText && tsPosClass !== "hidden" ? html`
          ...
          <div class="topbar">
            <div class="seg" role="tablist" aria-label="Filter">
              <button class="segbtn ${isToday ? "on" : ""}"
                @click=${() => {
                  this._selectedDay = newestDay; this._filterAll = false; this._selectedIndex = 0;
                  this._scrubMinute = NaN; this._pendingScrollToI = 0; this._exitSelectMode(); this.requestUpdate();
                }} title="${this._t("today")}" role="tab" aria-selected=${isToday}>
                <span>${this._t("today")}</span>
              </button>
            </div>

            <div class="datepill" role="group" aria-label="Datum navigatie">
              <button class="iconbtn" ?disabled=${!canPrev}
                @click=${() => this._stepDay(+1, days, currentForNav)} aria-label="${this._t("prev_day")}" title="${this._t("prev_day")}">
                <ha-icon icon="mdi:chevron-left"></ha-icon>
              </button>
              <div class="dateinfo" title="${this._t("selected_day")}">
                <span class="txt">${isAll ? this._t("all") : currentForNav ? this._formatDay(currentForNav) : "—"}</span>
              </div>
              <button class="iconbtn" ?disabled=${!canNext}
                @click=${() => this._stepDay(-1, days, currentForNav)} aria-label="${this._t("next_day")}" title="${this._t("next_day")}">
                <ha-icon icon="mdi:chevron-right"></ha-icon>
              </button>
            </div>

            ${showBulkToggle ? html`
              <button class="bulkbtn ${this._selectMode ? "on" : ""}"
                title=${this._selectMode ? this._t("stop_select") : this._t("select")}
                @click=${(e) => { e.preventDefault(); e.stopPropagation(); this._toggleSelectMode(); }}>
                <ha-icon icon="mdi:checkbox-multiple-marked-outline"></ha-icon>
              </button>
            ` : html``}

            <button class="bulkbtn" title="${this._t("download")}"
              @click=${(e) => { e.preventDefault(); e.stopPropagation(); this._downloadSrc(selected); }}>
          ...
          <div class="timeline">
            ${this._selectMode && (this._selectedSet?.size ?? 0) ? html`
              <div class="bulkbar topbulk">
                <span class="bulkcount">${this._selectedSet.size} ${this._t("selected")}</span>
                <div class="bulkactions">
                  <button type="button" class="bulkicon bulkcancel" title="${this._t("cancel")}" aria-label="${this._t("cancel")}"
                    @click=${(e) => { e.preventDefault(); e.stopPropagation(); this._exitSelectMode(); }}>
                    <ha-icon icon="mdi:close"></ha-icon>
                  </button>
                  <button type="button" class="bulkicon bulkdelete" title="${this._t("delete")}" aria-label="${this._t("delete")}"
            ...
            ${thumbs.length ? html`
              <div class="tthumbs" style="--tgap:${THUMB_GAP}px;">
                ${thumbs.map((it) => {
                  ...
                  return html`
                    <button class="tthumb ${isOn ? "on" : ""} ${this._selectMode && isSel ? "sel" : ""}"
                      data-i="${it.i}"
                      ...
                      @click=${(e) => {
                        ...
                        this._selectedIndex = it.i; this._scrubMinute = NaN;
                        this._pendingScrollToI = it.i; this.requestUpdate(); this._playPreviewIfVideo();
                      }}>
                      ${poster
